...
# Użycie: BASE_URL=http://localhost:8080 ./smoke.sh
set -e
...
CNT=$(curl -s -H "$A" $B/api/notes | jqget "" | python3 -c "import sys;print(len(eval(sys.stdin.read())))" 2>/dev/null || echo x)
...
# --- przypomnienia ---
R=$(curl -s -w '\n%{http_code}' -X POST $B/api/reminders -H "$A" -d '{"title":"Umówić przegląd auta","remindAt":"2026-07-30T09:00:00Z"}')
...
# --- wydarzenia ---
R=$(curl -s -w '\n%{http_code}' -X POST $B/api/events -H "$A" -d '{"title":"Fryzjer","startsAt":"2026-07-25T10:00:00Z","description":"u Magdy"}')
check "wydarzenie: create" 201 "$(tail -1 <<<"$R")"
R=$(curl -s -H "$A" "$B/api/events?from=2026-07-25T00:00:00Z&to=2026-07-26T00:00:00Z" | python3 -c "import sys,json;print(len(json.load(sys.stdin)))")
[ "$R" = "1" ] && check "wydarzenie: filtr zakresu" ok ok || check "wydarzenie: filtr zakresu" 1 "$R"

# --- obowiązki (z powtarzaniem) ---
R=$(curl -s -w '\n%{http_code}' -X POST $B/api/tasks -H "$A" -d '{"title":"Podlać kwiaty","dueDate":"2026-07-24","repeat":"weekly"}')
...
[ "$R" = "1 1 2026-07-31" ] && check "obowiązek: powtarzalny tworzy następny (+7 dni)" ok ok || check "obowiązek: powtarzalny tworzy następny" "1 1 2026-07-31" "$R"
...
R=$(curl -s -w '\n%{http_code}' -X PUT $B/api/mealplan -H "$A" -d "{\"date\":\"2026-07-25\",\"mealType\":\"obiad\",\"recipeId\":\"$RECIPE_ID\"}")
check "jadłospis: upsert z przepisem" 200 "$(tail -1 <<<"$R")"
R=$(curl -s -H "$A" "$B/api/mealplan?from=2026-07-25&to=2026-07-25" | python3 -c "import sys,json;d=json.load(sys.stdin);print(d[0]['recipeTitle'])")
[ "$R" = "Spaghetti bolognese" ] && check "jadłospis: join z tytułem przepisu" ok ok || check "jadłospis: join" "Spaghetti" "$R"

# --- dzienniczek ---
R=$(curl -s -w '\n%{http_code}' -X POST $B/api/diary -H "$A" -d '{"date":"2026-07-24","mealType":"sniadanie","description":"owsianka z malinami","calories":350}')
check "dzienniczek: create" 201 "$(tail -1 <<<"$R")"
R=$(curl -s -o /dev/null -w '%{http_code}' -X POST $B/api/diary -H "$A" -d '{"date":"2026-07-24","mealType":"złytyp","description":"x"}')
check "dzienniczek: walidacja typu posiłku" 400 "$R"

# --- budżet: limit, transakcje, podsumowanie ---
R=$(curl -s -o /dev/null -w '%{http_code}' -X PUT $B/api/budget/limits -H "$A" -d '{"month":"2026-07","category":"jedzenie","limitGrosze":150000}')
check "budżet: limit" 200 "$R"
R=$(curl -s -o /dev/null -w '%{http_code}' -X POST $B/api/transactions -H "$A" -d '{"occurredOn":"2026-07-24","kind":"expense","amountGrosze":15799,"category":"jedzenie","description":"Biedronka"}')
check "transakcja: wydatek" 201 "$R"
R=$(curl -s -o /dev/null -w '%{http_code}' -X POST $B/api/transactions -H "$A" -d '{"occurredOn":"2026-07-01","kind":"income","amountGrosze":520000,"description":"wypłata"}')
check "transakcja: przychód" 201 "$R"
R=$(curl -s -H "$A" "$B/api/budget/summary?month=2026-07" | python3 -c "
import sys,json; d=json.load(sys.stdin)
...
R=$(curl -s -o /dev/null -w '%{http_code}' -X POST $B/api/transactions -H "$A" -d "{\"occurredOn\":\"2026-07-24\",\"kind\":\"expense\",\"amountGrosze\":4999,\"category\":\"dom\",\"receiptFileId\":\"$FILE_ID\"}")
